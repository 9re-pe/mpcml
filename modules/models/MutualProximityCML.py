...
from ..adaptors.BaseAdaptor import BaseAdaptor
from ..distributions.BaseDistribution import BaseDistribution


class MutualProximityCML(CollaborativeMetricLearning):
    def __init__(
            self,
            n_user: int,
            n_item: int,
    ...
    def compute_users_distribution_params(self, users: torch.Tensor, n_pairs: int):
        ...
        if self.distribution.name == 'Empirical':
            sample_items = torch.tensor(range(self.n_item)).unsqueeze(1)
        else:
            sample_items = torch.tensor(sample_ids(self.n_item, self.n_item_sample)).unsqueeze(1)
        sample_items = sample_items.to(self.device)
        i_emb = self.item_embedding(sample_items)                                         # [n_sample, 1, n_dim]
        u_emb = self.user_embedding(users)[0].unsqueeze(0).repeat(i_emb.size()[0], 1, 1)  # [n_sample, 1, n_dim]
    ...
    def compute_items_distribution_params(self, items: torch.Tensor, n_pairs: int):
        ...
        if self.distribution.name == 'Empirical':
            sample_users = torch.arange(self.n_user).unsqueeze(0).expand(n_pairs, -1)
        else:
            sample_users = torch.zeros((n_pairs, self.n_user_sample), dtype=torch.int64)
            for i in range(n_pairs):
                row = sample_ids(self.n_user, self.n_user_sample)
                sample_users[i] = torch.tensor(row)

        i_emb = self.item_embedding(items)            # [n_pairs, 1, n_dim]
        sample_users = sample_users.to(self.device)
        u_emb = self.user_embedding(sample_users)     # [n_pairs, n_sample, n_dim]
        distances = compute_distance(i_emb, u_emb)    # [n_pairs, n_sample]
