...
class RecallEvaluator(UserwiseEvaluator):
    """Class of evaluators computing Recall for each user and calculating average."""
    ...
    def compute_score(
        ...
        if y_test_user.sum() == 0:
            ...
            df_eval_sub = pd.DataFrame(
                {
                    f"{self.metric_name}@{k}": [self.recall(y_test_user, y_hat_user, k)] for k in self.ks
                }
            )
